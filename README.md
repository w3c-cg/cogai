...
Today's global data centre energy consumption is scaling exponentially, driven relentlessly by generative AI workloads. This creates huge challenges for energy generation and distribution over national power grids. Hybrid photonics and 2D transistors will dramatically slash the power budget. The current data centre explosion risks becoming the most expensive case of premature infrastructure locking in tech history. Investors are building capital-intensive "power plants" for an architecture that is about to be replaced by low-energy alternatives. When ultra low power optical and 2D-transistor systems hit the market, these multi-billion-dollar facilities will effectively become legacy infrastructure overnight: functionally obsolete long before their 15-to-20-year depreciation cycles are complete.

If an artificial synapse requires only ‭$\sim1\text{ fJ}$‬‭‬ per spike, an advanced neural network could operate within a tiny power envelope. Smartphones, wearables, and autonomous drones could run trillions of parameter models locally on a tight battery budget, matching the 20W efficiency of the human brain. These devices wouldn't just run static, pre-trained models (inference); they could achieve continual, on-device learning. Your phone or smart glasses could continuously adapt to your habits, speech, and environment in real time, with zero latency and absolute data privacy, since no data ever leaves the device.
