...
Another promising approach features 2D transistors made of materials like Molybdenum Disulphide and Tungsten Diselenide. These are electronic switches that use ultra-thin, atomically flat nanomaterials a few atoms thick as their core conducting channel. As the size of silicon transistors shrink to sub-10 nanometer sizes, they suffer from electron leakage and waste heat. 2D transistors avoid this problem. Furthermore, 2D materials lack the unpaired electrons that trap and scatter electrons in silicon, allowing for incredibly high electron mobility, fast switching speeds and the promise of radically lower active power consumption.  2D materials can be conformally grown at temperatures below 400 °C, providing compatibility with complementary metal-oxide-semiconductor (CMOS) technology.

For integration in hybrid photonic systems, optical pulses can trap charge in floating gates. This directly mimics biological learning mechanisms like Short-Term and Long-Term Potentiation. By layering different 2D materials together (van der Waals heterostructures), researchers can minimize charge recombination. This allows a 2D optoelectronic transistor to process an incoming pulse using sub-femtojoule (< 1 fJ or 10⁻¹⁵ J) energy per spike, matching or even beating the efficiency of biological synapses in the human brain (~10 fJ).

Of course there are many challenges to address before these technology can become widespread. This includes surface passivation and physical encapsulation within a protective matrix, as well as connectivity to photonic chips compared to the ease of bonding wired connections. Traditional backpropagation requires global memory access to calculate gradients, which creates a massive data bottleneck. We are likely to need new architectures for neural networks that feature continual learning through continual prediction, and locally generated weight adjustments layer by layer. Another motivation is that adversarial attacks suggest that today’s AI is different from human cognition, e.g. adding some carefully chosen noise to an image of a panda causes the model to misclassify it as a gibbon with a high degree of confidence, despite the noise being imperceptible to the human eye. Similar considerations apply to prompt injection jail breaks that evade the alignment training for LLMs.

If an artificial synapse requires only 1 fJ‬‭‬ per spike, an advanced neural network could operate within a tiny power envelope. Smartphones, wearables, and autonomous drones could run trillions of parameter models locally on a tight battery budget, matching the 20W efficiency of the human brain. These devices wouldn't just run static, pre-trained models (inference); they could achieve continual, on-device learning. Your phone or smart glasses could continuously adapt to your habits, speech, and environment in real time, with zero latency and absolute data privacy, since no data ever leaves the device. Extended reality will become the norm with high-fidelity user experiences that blur the distinction between the real and the imagined, including humans and AI's.

Today's global data centre energy consumption is scaling exponentially, driven relentlessly by generative AI workloads. This creates huge challenges for energy generation and distribution over national power grids. Hybrid photonics and 2D transistors will dramatically slash the power budget. The rush to scale brute-force silicon infrastructure is setting the stage for the largest, most expensive asset-stranding event in tech history. When ultra low power optical and 2D-transistor systems hit the market, these multi-billion-dollar facilities risk becoming legacy infrastructure overnight: functionally obsolete long before their 15-to-20-year depreciation cycles are complete.
